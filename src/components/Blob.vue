  ...
  <div class="fixed -z-10 top-0 left-0 w-full h-screen">
    <div class="relative top-1/2 left-1/2 transform -translate-y-10">
      <div class="blob__container absolute transform">
        <div class="blob w-64 h-64 bg-yellow-300/30 rounded-full filter blur-xl transform -translate-x-1/2 -translate-y-[80%]" />
      </div>
      <div class="blob__container absolute transform">
        <div class="blob w-64 h-64 bg-blue-300/30 rounded-full filter blur-xl transform -translate-x-[80%] -translate-y-[20%]" />
      </div>
      <div class="blob__container absolute transform">
        <div class="blob w-64 h-64 bg-green-300/30 rounded-full filter blur-xl transform -translate-x-[20%] -translate-y-[20%]" />
      </div>
    </div>
  </div>
</template>
  ...
  setup() {
    onMounted(() => {
      const blobs = Array.from(document.querySelectorAll<HTMLDivElement>('.blob'), (blob) => Object.defineProperties({}, {
        scaleX: createCssPropPropertyDescriptor(blob, '--tw-scale-x'),
        scaleY: createCssPropPropertyDescriptor(blob, '--tw-scale-y'),
      }));
      const blobContainer = Array.from(document.querySelectorAll<HTMLDivElement>('.blob__container'), (blob) => Object.defineProperties({}, {
        rotate: createCssPropPropertyDescriptor(blob, '--tw-rotate'),
      }));
