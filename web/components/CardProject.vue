<template>
  <article class="m-2 w-full max-w-xs bg-white rounded-2xl shadow shadow-blue-300 transition-shadow hover:(shadow-xl)">
    <div class="overflow-hidden h-44 rounded-t-[inherit]">
      <img :src="thumbnailSrc" :alt="title" class="object-cover w-full h-full">
    </div>
    <div class="px-5 py-4">
      ...
      <div class="pt-1 flex justify-between">
        <a :href="url"
          class="flex items-center flex-nowrap gap-x-1 text-slate-300 font-medium transition-colors hover:text-blue-500">
        ...
        <div v-if="techStack.length"
          ...
          <img v-for="(stack, i) in techStack" :key="i" :src="stack" :alt="stack"
...
withDefaults(defineProps<Props>(), {
  url: '#',
  thumbnailSrc: 'https://via.placeholder.com/300',
  techStack: () => [],
});
</script>
